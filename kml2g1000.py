...
def export(kml, folder=None):
    ...
    for when, coord in zip(whens, coords):
        # Parse data (e.g. 2022-06-09T15:42:34.400527954Z)
        date, time = when.split('T')
        time = time.rstrip('Z')  # strip 'Z' and potential fractional seconds
        lng, lat, alt = coord.split(' ')

        # Calculate ground speed.
        to = (float(lat), float(lng))
        end = datetime.strptime(date + ' ' + time.split('.')[0], '%Y-%m-%d %H:%M:%S')
        ...
        csv.append(fmt.format(date=date, time=time.split('.')[0], lat=lat, lng=lng, alt=alt, gspd=gspd) + tail)
